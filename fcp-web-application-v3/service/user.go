...
// GetUserTaskCategory mengambil semua kategori tugas pengguna dari repositori.
func (s *userService) GetUserTaskCategory() ([]model.UserTaskCategory, error) {
	// Memanggil GetUserTaskCategory dari userRepo untuk mendapatkan daftar kategori tugas pengguna.
	listUserTaskCategory, err := s.userRepo.GetUserTaskCategory()
	if err != nil {
		// Jika terjadi error, kembalikan nil dan error tersebut.
		return nil, err
	}
	// Jika tidak ada error, kembalikan daftar kategori tugas pengguna.
	return listUserTaskCategory, nil
}
